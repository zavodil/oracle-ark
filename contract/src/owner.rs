...
impl Contract {
    ...
    /// Enable or disable contract-subsidized OutLayer calls
    /// When enabled and contract balance > 20 NEAR, oracle_call works without user deposit
    #[payable]
    pub fn set_subsidize_outlayer_calls(&mut self, enabled: bool) {
        assert_one_yocto();
        self.assert_owner();
        self.subsidize_outlayer_calls = enabled;
        log!(
            "Subsidize OutLayer calls: {}",
            if enabled { "enabled" } else { "disabled" }
        );
    }

    /// Check if OutLayer call subsidization is enabled
    pub fn get_subsidize_outlayer_calls(&self) -> bool {
        self.subsidize_outlayer_calls
    }

    /// Check if contract can currently subsidize OutLayer calls
    /// Returns true if subsidization is enabled AND balance > 20 NEAR
